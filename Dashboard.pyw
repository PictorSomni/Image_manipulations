        ...
        if event.data == "close":
            ...
            if proc is not None and proc.poll() is None:
                try:
                    ...
                    pass
            hotkey_listener = _mic_state.get("hotkey_listener")
...
# ===================== ÉLÉMENTS UI ===================== #
    ...
    ai_container = ft.Container(
        content=ft.Column([
            ...
            ai_progress_bar,
            ft.Row(
                [ai_attach_button, ai_input_field, ai_mic_button,
                 ai_send_button],
                spacing=4,
...
# ===================== MÉTHODES ===================== #
    ...
    def _copy_scored_photos_click(e=None):
        """Copie les photos ≥ AI_PHOTO_SCORE_THRESHOLD (score_photos) vers
        AI_PHOTO_SCORE_SELECTION_FOLDER, d'après le fichier de scores du
        dossier courant. Action déterministe déclenchée par Charles — pas
        un outil IA, la copie n'est jamais décidée par le modèle seul."""
        ...
        if not folder or not os.path.isdir(folder):
            ...
            return
        log_to_terminal("[SCORE IA] Copie de la sélection en cours…", YELLOW)

        def _run():
            summary = _copy_scored_photos(folder)
            log_to_terminal(f"[SCORE IA] {summary}", YELLOW)
            page.pubsub.send_all_on_topic("refresh", None)
    ...
    def _send_ai_message(message_text):
        ...
        def _run():
            ...
            try:
                    ...
                    for tc in tool_calls:
                        ...
                        if fn_name == "web_search":
                            ...
                            if not _score_candidates:
                                ...
                                _score_total   = len(_score_candidates)
                                _score_model   = ai_model_dropdown.value or CONSTANTS.AI_MODEL_VISION
                                _score_batch_n = (
                                    CONSTANTS.AI_GEMINI_FOLDER_BATCH_SIZE
                                    if (_score_model or "").startswith("gemini")
                                    else CONSTANTS.AI_FOLDER_SELECT_BATCH_SIZE
                                )
                                _score_batches = (_score_total + _score_batch_n - 1) // _score_batch_n
                                _score_progress_ctrl = _ai_add_bubble(
                                    "assistant",
                                    f"🏆 Score de {_score_total} image(s) — lot 1/{_score_batches}…",
                                )
                                _turn_events.append(f"🏆 Score de {_score_total} image(s)")
                ...
                _log_user_text = locals().get('_log_user_text') or next(
                    (m["content"] for m in reversed(ai_conversation) if m["role"] == "user"),
                    message_text
                )
                if _turn_events:
                    ...
                    _ai_save_history()
                _log_exchange_to_md(_log_user_text, f"[ERREUR] {exc}", _thinking, _turn_events)
            finally:
                ai_streaming["value"] = False
                ai_progress_bar.visible = False
                ai_stop_button.icon_color = LIGHT_GREY
    ...
    def _mic_start():
        ...
        try:
            ...
            log_to_terminal(f"[ERREUR] Micro : {mic_error}", RED)
        ...
        # Windows) : évite de perdre le début de la phrase.
    ...
    def _mic_stop(auto_send=False):
        ...
        def _worker():
            ...
            try:
                ...
                if wav:
                    # Diagnostic : durée réellement captée + sauvegarde du WAV
                    # (permet de distinguer un souci de capture d'un souci de
                    ...
                    try:
                        ...
                        elapsed = getattr(recorder, "elapsed", 0.0)
                        overflows = getattr(recorder, "_overflows", 0)
                        log_to_terminal(
                            f"[IA] Dictée : {dur:.1f}s captées @ {rate} Hz "
                            f"(bouton tenu {elapsed:.1f}s, {overflows} "
                            f"overflow(s)) → {dbg}", BLUE)
                    except Exception:
                        pass
                ...
                log_to_terminal(f"[ERREUR] Transcription : {stt_error}", RED)
    ...
    def _mic_hotkey_start():
        ...
        def _on_press(key):
            if _is_ptt(key) and not _mic_state["side_panel_priority"]:
    ...
    async def _ai_paste_clipboard(auto_send=False):
        ...
        try:
            ...
            log_to_terminal(f"[ERREUR] Presse-papiers : {paste_error}", RED)
            return
    ...
    def _on_ai_submit():
        ...
        # Autoriser aussi l'envoi sans texte si des images/fichiers sont joints.
