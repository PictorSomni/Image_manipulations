...
if dest_dir:
    ...
    copied_basenames = {Path(strip_copies_prefix(f)).stem.lower() for f in copied_files}
    missing_files = [file for file in all_files if Path(file).stem.lower() not in copied_basenames and not file.startswith('.') and not file.endswith('.py')]
    ...
    if parent_dir == PATH or not parent_dir.is_dir():
        ...
        parent_files = {Path(strip_copies_prefix(file.name)).stem.lower() for file in parent_dir.iterdir() if file.is_file()}
        extra_files = [f for f in current_files if Path(strip_copies_prefix(f)).stem.lower() not in parent_files and not f.startswith('.') and not f.endswith('.py')]
