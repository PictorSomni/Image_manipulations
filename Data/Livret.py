...
cheval) : les pages sont réparties 2 par feuille, dans l'ordre qui, une
fois les feuilles imprimées en recto-verso puis pliées/agrafées au
centre, redonne la lecture 1, 2, 3... N. Contrairement au mode "livret"
du pilote d'imprimante, les images ne sont PAS redimensionnées : elles
...
nombre de pages n'est pas un multiple de 4, des pages blanches sont
insérées juste avant la dernière (pour ne jamais déplacer les
couvertures) jusqu'à tomber juste.
...
n = len(pages)
sheets = n // 4
print(f"{n} pages -> {sheets} feuille(s) recto-verso, 2 pages/face")

# Imposition piqûre à cheval : la 1re feuille imprimée porte les
# couvertures (page 1 et page n), et ainsi de suite vers le centre —
# c'est l'algorithme standard des logiciels de "booklet printing"
