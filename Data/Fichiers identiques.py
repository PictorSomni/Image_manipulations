...
def _compare(source: Path, destination: Path) -> list[str]:
    """Retourne la liste des fichiers présents à la fois dans source et dans destination."""
    ...
    return [
        ...
        if Path(file).stem.lower() in dest_basenames
...
async def main(page: ft.Page) -> None:
    page.title = "Fichiers identiques"
    ...
    def run_comparison(e) -> None:
        ...
        common_files = _compare(source_path, dest_path)
        common_str = "|".join(common_files)
        os.environ[ENV_SELECTED_FILES_KEY] = common_str
        print(f"{len(common_files)} fichier(s) identique(s) dans {dest_path.name}.")
        print(f"{OUTPUT_SELECTED_FILES_PREFIX}{common_str}")
        ...
        ft.Container(
            content=ft.Column([
                ft.Row([
                    ...
                    ft.Text("Fichiers identiques", size=16, color=WHITE, weight=ft.FontWeight.W_600),
...
# Mode sans interface : si toutes les données sont disponibles depuis Hub
