...
for i, file in enumerate(FOLDER) :
    ...
    try :
        ...
        # gamma = (1/(np.argmax(img_output)/256))
        # gamma = (1/1-(np.average(img_output)/256))
        gamma = (1/(np.average(img_output)/256))#**2

        print(gamma)
        result = cv2.addWeighted(img, 0.6, img_output, 0.4, gamma)
